...
let queryString1 = "select * from scheduleCSV where Instructors in (select name from professorData where isAdjunct= 0) and (Course like \'%CPSC%\' or Course like \'%DATA%\' or Course like \'%CYBR%\' or Course like \'%FSEM%\');";
let queryString2 = "select * from professorData where isAdjunct= 0;";
let queryString3 = "select * from scheduleCSV where Instructors in (select name from professorData where isAdjunct= 1) and (Course like \'%CPSC%\' or Course like \'%DATA%\' or Course like \'%CYBR%\' or Course like \'%FSEM%\');";
let queryString4 = "select * from professorData where isAdjunct= 1;";
var d = new Date();
...
con.query(queryString1, function(err, courseData, fields) {
    ...
    con.query(queryString2, function(err, profData, fields) {
        ...
        tableHTML = tableHTML.concat('<tr>');
        tableHTML = tableHTML.concat('<th rowspan="2" scope="col">');
        tableHTML = tableHTML.concat('Adjuncts');
        tableHTML = tableHTML.concat('<a href="prof_data.html"><img class="icon" src="images/edit_icon.png" height="25px" width="25px"></a>');
        tableHTML = tableHTML.concat('</th>');
        tableHTML = tableHTML.concat('<th colspan="5" scope="colgroup">Office Hours</th>');
        tableHTML = tableHTML.concat('<th rowspan="2" scope="col">Course(s)</th>');
        ...
        con.query(queryString3, function(err, courseData2, fields) {
            if (err) throw err;
            con.query(queryString4, function(err, profData2, fields) {
                if (err) throw err;
                for (let i = 0; i < profData2.length; i++) {

                    // Build an array full of our professor data
                    let professor2 = profData2[i];
                    let profCourses2 = [];
                    for (let x = 0; x < courseData2.length; x++) {
                        if (courseData2[x].Instructors == professor2.name) {
                            profCourses2.push(courseData[x]);
                        }
                    }
                    ...
                    // Init empty office hour cells
                    tableHTML = tableHTML.concat('<tr>');
                    tableHTML = tableHTML.concat('<td rowspan="' + (profCourses2.length + 1) + '">' + professor2.name + '</td>');
                    for (let i = 0; i < 5; i++) {
                        tableHTML = tableHTML.concat('<td rowspan="' + (profCourses2.length + 1) + '"></td>');
                    }
                    tableHTML = tableHTML.concat('</tr>');

                    // Make a <tr> for each class this prof has
                    for (let i = 0; i < profCourses2.length; i++) {
                        tableHTML = tableHTML.concat('<tr>');

                        tableHTML = tableHTML.concat('<td>' + profCourses2[i].Course + '</td>');
                        tableHTML = tableHTML.concat('<td>' + profCourses2[i].TIME + ' (' + profCourses2[i].Days + ')</td>');
                        tableHTML = tableHTML.concat('<td>' + profCourses2[i].Bldgs + ' ' + profCourses2[i].Rooms + '</td>');

                        tableHTML = tableHTML.concat('</tr>');
                    }

                    tableHTML = tableHTML.concat('<tr><td colspan="9">' + professor2.office + ' - ' + professor2.phone + ' - ' + professor2.email + '</td></tr>');
                    tableHTML = tableHTML.concat('</tbody>');
